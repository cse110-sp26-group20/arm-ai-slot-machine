...
def main():
    if len(sys.argv) != 3:
        print("Usage: python timestamp.py <number> <refinement number")
        sys.exit(1)

    start, num = int(sys.argv[1]),int(sys.argv[2])
    

    folder = f"candidate-{start:03d}-refinement-{num:d}"
    ...
    dt_la = (dt - timedelta(hours=7)).replace(second=0, tzinfo=timezone(timedelta(hours=-7)))
